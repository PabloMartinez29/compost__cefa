...
<div class="container mx-auto px-6 py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('aprendiz.composting.create') }}" class="bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg transition-all duration-200 flex items-center shadow-lg hover:shadow-xl hover:from-green-600 hover:to-green-700">
                            ...
                            <td class="py-4 px-6 border-b border-gray-100">
                                <div class="flex space-x-2 items-center">
                                    ...
                                    @if($composting->created_by === auth()->id())
                                        <a href="{{ route('aprendiz.composting.edit', $composting) }}" 
                                        ...
                                        <button id="deleteBtn{{ $composting->id }}" onclick="requestDeletePermission({{ $composting->id }})" 
                                           class="inline-flex items-center justify-center w-10 h-10 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all duration-200" title="Solicitar Eliminación">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    @else
                                        <button onclick="requestEditPermission({{ $composting->id }})" 
                                           class="inline-flex items-center justify-center w-10 h-10 text-yellow-500 hover:text-yellow-700 hover:bg-yellow-50 rounded-lg transition-all duration-200" title="Solicitar Edición">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button id="deleteBtn{{ $composting->id }}" onclick="requestDeletePermission({{ $composting->id }})" 
                                           class="inline-flex items-center justify-center w-10 h-10 text-orange-500 hover:text-orange-700 hover:bg-orange-50 rounded-lg transition-all duration-200" title="Solicitar Eliminación">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    @endif
                                </div>
...
function openViewModal(compostingId) {
    ...
    fetch(`/aprendiz/composting/${compostingId}`, {
        method: 'GET',
        headers: {
...
function requestEditPermission(compostingId) {
    Swal.fire({
        title: 'Solicitar permiso de edición',
        text: '¿Desea solicitar permiso al administrador para editar esta pila?',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/aprendiz/composting/${compostingId}/request-edit`;
...
function requestDeletePermission(compostingId) {
    console.log('Checking delete permission status for composting ID:', compostingId);
    
    // Primero verificar el estado de la solicitud
    fetch(`/aprendiz/composting/${compostingId}/check-delete-status`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
    ...
    .then(data => {
        ...
        if (data.has_request) {
            console.log('Request exists with status:', data.status);
            // Ya existe una solicitud, mostrar alerta informativa
            if (data.status === 'pending') {
                ...
                // Solicitud aprobada, mostrar modal de confirmación de eliminación directa
                Swal.fire({
                    title: 'Eliminar Pila',
                    text: 'Su solicitud fue aprobada. ¿Desea eliminar esta pila de compostaje?',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#dc2626',
                    cancelButtonColor: '#6b7280',
                    ...
                    if (result.isConfirmed) {
                        console.log('User confirmed deletion for composting ID:', compostingId);
                        // Crear formulario para eliminar directamente
                        const form = document.createElement('form');
                        form.method = 'POST';
                        form.action = `/aprendiz/composting/${compostingId}`;
                        ...
                        form.appendChild(methodField);
                        document.body.appendChild(form);
                        console.log('Form created and submitted to:', form.action);
                        form.submit();
                    }
                });
            ...
            console.log('No request exists, showing confirmation modal');
            // No hay solicitud, mostrar modal de confirmación
            Swal.fire({
                title: 'Solicitar permiso de eliminación',
                text: '¿Desea solicitar permiso al administrador para eliminar esta pila?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#dc2626',
                cancelButtonColor: '#6b7280',
                ...
                if (result.isConfirmed) {
                    console.log('User confirmed, creating form for composting ID:', compostingId);
                    // Crear formulario para solicitar permiso
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = `/aprendiz/composting/${compostingId}/request-delete`;
    ...
    .catch(error => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                console.log('User confirmed (catch), creating form for composting ID:', compostingId);
                // Crear formulario para solicitar permiso
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `/aprendiz/composting/${compostingId}/request-delete`;
                ...
                form.appendChild(tokenField);
                document.body.appendChild(form);
                console.log('Form created and submitted to (catch):', form.action);
                form.submit();
            }
        });
    });
...
// Función para cargar el estado de los botones de eliminación
function loadDeleteButtonStates() {
    // Obtener todos los botones de eliminación
    const deleteButtons = document.querySelectorAll('[id^="deleteBtn"]');
    
    deleteButtons.forEach(button => {
        const compostingId = button.id.replace('deleteBtn', '');
        ...
        .then(data => {
            if (data.has_request && data.status === 'rejected') {
                // Cambiar el icono a prohibido/rechazo
                const icon = button.querySelector('i');
                icon.className = 'fas fa-ban';
                
                // Cambiar colores a rojo (rechazado)
                button.className = button.className.replace(/text-\w+-\d+/, 'text-red-500')
                                                   .replace(/hover:text-\w+-\d+/, 'hover:text-red-700')
                                                   .replace(/hover:bg-\w+-\d+/, 'hover:bg-red-50');
                
                // Cambiar el título
                button.title = 'Solicitud Rechazada';
        ...
        .catch(error => {
            console.log('Error checking delete status for composting ID:', compostingId, error);
        });
    });
}

// Cargar estados cuando se carga la página
document.addEventListener('DOMContentLoaded', function() {
    loadDeleteButtonStates();
});
</script>
@endsection
