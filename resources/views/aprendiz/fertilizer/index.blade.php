                            ...
                            <td>
                                <div class="flex space-x-2 items-center">
                                    ...
                                    @if($fertilizer->composting && $fertilizer->composting->created_by == auth()->id())
                                        ...
                                        @php
                                            $isApproved = isset($approvedFertilizerIds) && in_array($fertilizer->id, $approvedFertilizerIds);
                                            $isPending = isset($pendingFertilizerIds) && in_array($fertilizer->id, $pendingFertilizerIds);
                                            $isRejected = isset($rejectedFertilizerIds) && in_array($fertilizer->id, $rejectedFertilizerIds);
                                        @endphp

                                        @if($isRejected)
                                            <button type="button" class="inline-flex items-center text-red-600 hover:text-red-800" title="Solicitud rechazada"
                                                onclick="showRejectedAlert({{ $fertilizer->id }})">
                                                <i class="fas fa-ban text-lg"></i>
                                            </button>
                                        @elseif($isApproved)
                                            <form id="delete-form-{{ $fertilizer->id }}" action="{{ route('aprendiz.fertilizer.destroy', $fertilizer) }}" method="POST" class="inline-flex items-center" style="margin: 0; padding: 0; margin-left: 0.5rem;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="inline-flex items-center text-red-500 hover:text-red-700" title="Eliminar"
                                                    onclick="confirmDelete('delete-form-{{ $fertilizer->id }}')">
                                        ...
                                        @else
                                            <form id="request-delete-form-{{ $fertilizer->id }}" action="{{ route('aprendiz.fertilizer.request-delete', $fertilizer) }}" method="POST" class="inline-flex items-center" style="margin: 0; padding: 0; margin-left: 0.5rem;">
                                                @csrf
                                                <button type="button" 
                                                   class="inline-flex items-center text-red-500 hover:text-red-700" title="Solicitar Eliminación"
                                                   onclick="confirmRequestPermission('request-delete-form-{{ $fertilizer->id }}')">
                                                    <i class="fas fa-trash"></i>
                                                </button>
                                            </form>
                                        @endif
                                    ...
                                    @endif
                                    <a href="{{ route('aprendiz.fertilizer.download.pdf', $fertilizer) }}" 
        ...
        <div class="waste-header">
            <div class="text-center">
                ...
                <p class="waste-subtitle">
                    ...
                    {{ Auth::user()?->name ?? 'Usuario' }} - Registro #<span id="viewRecordId"></span>
                </p>
            </div>
        </div>
        <div class="p-6">
...
function openViewModal(fertilizerId) {
    fetch(`/aprendiz/fertilizer/${fertilizerId}`, {
        headers: {
...
function openEditModal(fertilizerId) {
    fetch(`/aprendiz/fertilizer/${fertilizerId}/edit`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
        }
        ...
        .then(result => {
            ...
            editForm.action = `/aprendiz/fertilizer/${fertilizerId}`;
...
function showPermissionAlert() {
    Swal.fire({
        title: 'Sin Permisos',
        text: 'No tiene permisos para realizar esta acción en este registro.',
        icon: 'warning',
        confirmButtonColor: '#6b7280'
    });
}

function showRejectedAlert(fertilizerId) {
    Swal.fire({
        title: 'Solicitud Rechazada',
        text: 'Su solicitud de eliminación fue rechazada por el administrador. Puede solicitar nuevamente si lo desea.',
        icon: 'error',
        confirmButtonColor: '#6b7280'
    });
}
...
@if(session('permission_required'))
    Swal.fire({
        title: 'Atención',
        text: '{{ session('permission_required') }}',
        icon: 'warning',
