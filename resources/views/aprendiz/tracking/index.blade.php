...
@php
    ...
    $rejectedTrackingIds = $rejectedTrackingIds ?? [];
    $currentUserId = auth()->id();
@endphp

<script>
// IDs de notificaciones para verificar permisos
const approvedTrackingIds = @json($approvedTrackingIds);
const pendingTrackingIds = @json($pendingTrackingIds);
const rejectedTrackingIds = @json($rejectedTrackingIds);
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ${allDays.map(item => {
                                        if (item.type === 'tracking') {
                                            ...
                                            const isOwner = tracking.created_by == currentUserId;
                                            const isPileCreatedByAdmin = tracking.pile_created_by_admin || false;
                                            const isApproved = approvedTrackingIds.includes(tracking.id);
                                            const isPending = pendingTrackingIds.includes(tracking.id);
                                            const isRejected = rejectedTrackingIds.includes(tracking.id);
                                            ...
                                            // Si no es el dueño, no mostrar botón de eliminar
...
function requestDeletePermission(trackingId) {
    Swal.fire({
        title: '¿Solicitar permiso para eliminar?',
        text: 'Se enviará una solicitud al administrador para eliminar este seguimiento',
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/aprendiz/tracking/${trackingId}/request-delete-permission`;
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="p-3 sm:p-4 md:p-6 border-b border-gray-200 bg-gray-50">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                ...
                <a href="{{ route('aprendiz.tracking.download.all-pdf') }}" 
...
function openViewTrackingModal(trackingId) {
    fetch(`/aprendiz/tracking/${trackingId}`, {
        headers: {
        ...
        .then(data => {
            ...
            // Construir contenido (mismo que admin)
            let content = `
...
function openEditTrackingModal(trackingId) {
    fetch(`/aprendiz/tracking/${trackingId}/edit`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest',
            'Accept': 'application/json'
        }
        ...
        .then(result => {
            ...
            document.getElementById('editTrackingForm').action = `/aprendiz/tracking/${trackingId}`;
