...
<div class="waste-container">
    ...
    <div class="waste-header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.tracking.index') }}" class="bg-soft-gray-600 hover:bg-soft-gray-700 text-white p-3 rounded-lg transition-colors duration-200" title="Volver">
    ...
    <div class="bg-white rounded-xl shadow-xl border border-soft-gray-100 overflow-hidden">
        <form action="{{ route('admin.tracking.update', $tracking) }}" method="POST" class="p-8">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-6">
                        ...
                        <select name="composting_id" id="composting_id" required
                            ...
                            @foreach($activeCompostings as $composting)
                                <option value="{{ $composting->id }}" 
                                        {{ (old('composting_id', $tracking->composting_id) == $composting->id) ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div class="bg-soft-gray-50 rounded-lg p-4 border border-soft-gray-200">
                            ...
                            <input type="date" name="date" id="date" required
                                   value="{{ old('date', $tracking->date->format('Y-m-d')) }}"
                    ...
                    <div class="bg-soft-gray-50 rounded-lg p-4 border border-soft-gray-200">
                        ...
                        <textarea name="activity" id="activity" rows="3" required
                                  ...
                                  placeholder="Describe las actividades realizadas...">{{ old('activity', $tracking->activity) }}</textarea>
                        @error('activity')
                            <p class="text-red-500 text-sm mt-2 flex items-center">
                                <i class="fas fa-exclamation-circle mr-1"></i>
                                {{ $message }}
                    ...
                    <div class="bg-soft-gray-50 rounded-lg p-4 border border-soft-gray-200">
                        ...
                        <input type="text" name="work_hours" id="work_hours" required
                               value="{{ old('work_hours', $tracking->work_hours) }}"
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-soft-gray-50 rounded-lg p-4 border border-soft-gray-200">
                        ...
                        <textarea name="others" id="others" rows="3"
                                  ...
                                  placeholder="Observaciones, notas adicionales...">{{ old('others', $tracking->others) }}</textarea>
                        @error('others')
                            <p class="text-red-500 text-sm mt-2 flex items-center">
                                <i class="fas fa-exclamation-circle mr-1"></i>
                                {{ $message }}
