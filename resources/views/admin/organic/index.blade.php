        ...
        <div class="p-3 sm:p-4 md:p-6 border-b border-gray-200 bg-gray-50">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-4">
                ...
                <div class="flex items-center space-x-2 sm:space-x-4 w-full sm:w-auto">
                    ...
                    @endif
                    <a href="{{ route('admin.organic.create') }}" class="bg-green-400 text-green-800 border border-green-500 hover:bg-green-500 px-3 sm:px-4 py-2 rounded-lg transition-all duration-200 flex items-center shadow-sm text-sm sm:text-base flex-1 sm:flex-initial justify-center">
                ...
                @foreach($organics as $organic)
                    <div class="waste-mobile-card bg-gray-50 border border-gray-200 rounded-xl p-4 shadow-sm" data-id="{{ $organic->id }}">
                        ...
                        <div class="waste-mobile-card-actions mt-4 pt-3 border-t border-gray-200">
                            ...
                            <form action="{{ route('admin.organic.destroy', $organic) }}" method="POST" class="inline flex-shrink-0" onsubmit="return confirmDelete(event, this)">@csrf @method('DELETE')<button type="submit" class="p-2 text-red-500 hover:bg-red-50 rounded-lg" title="Eliminar"><i class="fas fa-trash"></i></button></form>
                            <a href="{{ route('admin.organic.download.pdf', $organic) }}" class="p-2 text-red-700 hover:bg-red-50 rounded-lg flex-shrink-0" title="PDF"><i class="fas fa-file-pdf"></i></a>
                        </div>
                    </div>
                @endforeach
            </div>

            <!-- Tabla de residuos (escritorio) -->
